...
1、字典表 td_ 开头,也有 tc_开头的，如 td_city， tc_code，这里的数据基本是手工配置，和业务流程无关，常常和业务表做关联拥有描述

2、关系表 tr_ 开头， 如 tr_user_role ，它保存单纯的关系数据，不参与更多的逻辑

3、描述表 tl_ 开头，如 tl_login_log 登录日志

4、统计表 ts_ 开头，如 ts_product_sale ，它从其他表中计算得出统计数据，用于报表或展示

5、业务表 tt_ 开头

6、主数据 tm_ 开头，tm_user
