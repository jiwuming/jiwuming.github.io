...
# 队列
所谓队列就是指一连串的任务, 这些任务可以顺序执行, 也可以同时执行。这也就对应我们平时说的串行队列和并行队列。`GCD`官方文档一开始就给我们介绍了两种队列
...
```
系统所定义的一种全局队列, 可以通过制定优先级去完成不同的任务, 它默认是一个并行队列。
...
```
根据文档, 我们可以得出第一个参数是一个唯一标识符用于调试和定位, 也可以为空的, 第二个参数是队列类型, 有串行和并行。
...
dispatch_sync(syncConcurrentQueue, ^{
    NSLog(@"同步 并行 1 %@", [NSThread currentThread]);
});

dispatch_sync(syncConcurrentQueue, ^{
    [NSThread sleepForTimeInterval:2];
    NSLog(@"同步 并行 2 %@", [NSThread currentThread]);
});

NSLog(@"中间 %@", [NSThread currentThread]);

dispatch_sync(syncConcurrentQueue, ^{
    NSLog(@"同步 并行 3 %@", [NSThread currentThread]);
});

dispatch_sync(syncConcurrentQueue, ^{
    NSLog(@"同步 并行 4 %@", [NSThread currentThread]);
});
...
dispatch_async(asyncConcurrentQueue, ^{
    NSLog(@"异步 并行 1 %@", [NSThread currentThread]);
});

dispatch_async(asyncConcurrentQueue, ^{
    [NSThread sleepForTimeInterval:2];
    NSLog(@"异步 并行 2 %@", [NSThread currentThread]);
});

NSLog(@"中间 %@", [NSThread currentThread]);

dispatch_async(asyncConcurrentQueue, ^{
    NSLog(@"异步 并行 3 %@", [NSThread currentThread]);
});

dispatch_async(asyncConcurrentQueue, ^{
    NSLog(@"异步 并行 4 %@", [NSThread currentThread]);
});
...
异步 并行 1 <NSThread: 0x280090380>{number = 5, name = (null)}

***** 时间01:39:22 GCDViewController.m 第50行 *****
结束 <NSThread: 0x2800c6100>{number = 1, name = main}

***** 时间01:39:22 GCDViewController.m 第43行 *****
异步 并行 3 <NSThread: 0x28009b700>{number = 6, name = (null)}

***** 时间01:39:22 GCDViewController.m 第47行 *****
异步 并行 4 <NSThread: 0x28009b700>{number = 6, name = (null)}

***** 时间01:39:24 GCDViewController.m 第37行 *****
异步 并行 2 <NSThread: 0x280090380>{number = 5, name = (null)}
```
异步+并行这种方式是真正的并行+多线程, 有堵塞的任务会最后执行, 且线程都不相同 
...
NSLog(@"结束了");
```
利用`dispatch_barrier_async`或`dispatch_barrier_sync`都可达到效果。但个人感觉这个东西没太大用处, 先记录一下, 可能某天会遇到呢。

### 小结
* 同步执行, 不会开启新的线程, 无论是串行或并行的同步任务, 都以串行的方式执行
* 异步执行, 会开启新的线程, 串行即一个一个的执行, 并行及并发执行
